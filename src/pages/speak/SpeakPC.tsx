...
export default function SpeakPC({
  ...
  const saveResult = async (score: number) => {
    const lastQuiz = speakData.filter(
      (data, i) =>
        data.Sentence !== null && i > quizIndex && data.Sequence !== 999,
    )
  ...
  const maxLength = speakData.filter(
    (d) => d.QuizNo !== null && d.QuizNo > 0,
  ).length

  const progressWidth = (speakData[quizIndex].QuizNo / maxLength) * 100
