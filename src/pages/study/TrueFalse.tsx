  ...
  const checkAnswer = async (target: HTMLDivElement, selectedBtn: boolean) => {
    try {
      if (!isWorking.current) {
        ...
        const trueText = quizData.Quiz[quizNo - 1].Examples[0].Text

        // 해당 문항이 정답인지
        const isCorrectText = question === trueText
        ...
        const userAnswer: IUserAnswer = makeUserAnswerData({
          ...
          correct: trueText.replace(replaceHTMLReg, ''),
          selectedAnswer: isCorrect
            ? trueText.replace(replaceHTMLReg, '')
            : 'user has the wrong opinion.',
          tryCount: tryCount + 1,
    ...
    try {
      if (!isWorking.current) {
        ...
        const userAnswer: IUserAnswer = makeUserAnswerData({
          ...
          selectedAnswer: 'user has the wrong opinion',
          tryCount: tryCount + 1,
          maxQuizCount: quizData.QuizAnswerCount,
          quizLength: quizData.Quiz.length,
          isCorrect: isCorrect,
