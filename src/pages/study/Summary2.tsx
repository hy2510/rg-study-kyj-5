...
import { useCurrentQuizNoSummary2 } from '@hooks/study/useCurrentQuizNo'
...
export default function Summary2(props: IStudyData) {
  ...
  useEffect(() => {
    if (quizData) {
      ...
      if (recordedData && recordedData.length > 0) {
        ...
        ;[currentQuizNo, lastQuizNo, tryCnt] = useCurrentQuizNoSummary2(
          studyInfo.mode,
          recordedData,
          requireQuizCount,
      ...
      const convertedData = convertPartialRecordToScoreBoardSummary2(
        recordedData,
        quizData.QuizAnswerCount,
      )

      addStudentAnswers([...convertedData])
  ...
  const maxQuizLength = quizData.Quiz.reduce((acc, cur) => {
