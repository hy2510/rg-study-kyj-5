...
export default function AppContextProvider({
      ...
      .then((response) => {
        ...
        if (response.studyInfo.isStartSpeak) {
          ...
          if (
            ...
            if (response.studyInfo.isReview) {
              setIsPreference(true)
            }
  ...
  const contextHandler = {
    ...
    changeReadingComplete: async (
      studyId: string,
      studentHistoryId: string,
    ) => {
      if (!isReadingComplete || storyMode === 'Story') {
        const result = await submitReadLastPage(studyId, studentHistoryId)
